...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := bitcoincash_addrdec.BCH_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = bitcoincash_addrdec.BCH_testnetPrivateWIFCompressed
	}

	wif, _ := bitcoincash_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	...
	address := addressEncoder.AddressEncode(hash, addressEncoder.BCH_mainnetAddressCash)
	//bitcoincash_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	//address, err := bitcoincash_addrdec.Default.AddressEncode(pub)
	//if err != nil {
	//	return "", err
	//}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := bitcoincash_addrdec.BCH_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = bitcoincash_addrdec.BCH_testnetAddressP2SH
	}
	...
	address, _ := bitcoincash_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := bitcoincash_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
