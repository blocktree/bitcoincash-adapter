...
func (decoder *TransactionDecoder) CreateBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if len(rawTx.FeeRate) == 0 {
		feesRate = decoder.wm.Config.MinFees
...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := bitcoincashTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &bitcoincashTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: bitcoincashTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := bitcoincashTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]bitcoincashTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]bitcoincashTransaction.SignaturePubkey, 0)
		transHash     = make([]bitcoincashTransaction.TxHash, 0)
		addressPrefix bitcoincashTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := bitcoincashTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := bitcoincashTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		amount, _  := decimal.NewFromString(utxo.Value)
		amount = amount.Shift(decoder.wm.Decimal())

		txUnlock := bitcoincashTransaction.TxUnlock{
	...
	signedTrans, err := bitcoincashTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := bitcoincashTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) CreateOmniRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		...
		availableUTXO     = make([]*bitcoin.Unspent, 0)
		usedUTXO          = make([]*bitcoin.Unspent, 0)
	...
	*/
	...
	//获取utxo，按小到大排序
	sort.Sort(bitcoin.UnspentSort{availableUTXO, func(a, b *bitcoin.Unspent) int {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]bitcoincashTransaction.Vin, 0)
		vouts            = make([]bitcoincashTransaction.Vout, 0)
		txUnlocks        = make([]bitcoincashTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := bitcoincashTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		amount, _  := decimal.NewFromString(utxo.Amount)
		amount = amount.Shift(decoder.wm.Decimal())

		txUnlock := bitcoincashTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: bitcoincashTransaction.SigHashAll, Amount:uint64(amount.IntPart())}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		amount = amount.Shift(decoder.wm.Decimal())
		out := bitcoincashTransaction.Vout{to, uint64(amount.IntPart())}
	...
	for i := 0; i < len(vouts); i ++ {
		vouts[i].Address, err = bitcoincash_addrdec.ConvertToLegacy(vouts[i].Address)
		if err != nil {
			return fmt.Errorf("create transaction failed, unexpected error: %v", err)
		}
	}
	/////////构建空交易单
	emptyTrans, err := bitcoincashTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := bitcoincashTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: bitcoincashTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
...
func (decoder *TransactionDecoder) CreateOmniSummaryRawTransaction(wrapper openwallet.WalletDAI, sumRawTx *openwallet.SummaryRawTransaction) ([]*openwallet.RawTransactionWithError, error) {
	...
	var (
		...
		feesSupportUnspents []*bitcoin.Unspent
	)
...
func (decoder *TransactionDecoder) keepOmniCostUTXONotToUse(unspents []*bitcoin.Unspent) []*bitcoin.Unspent {
	...
	var (
		...
		resultUTXO = make([]*bitcoin.Unspent, 0)
	)
...
// getAssetsAccountUnspentSatisfyAmount
func (decoder *TransactionDecoder) getUTXOSatisfyAmount(unspents []*bitcoin.Unspent, amount decimal.Decimal) (*bitcoin.Unspent, *openwallet.Error) {

	var utxo *bitcoin.Unspent
...
// removeUTXO
func removeUTXO(slice []*bitcoin.Unspent, elem *bitcoin.Unspent) []*bitcoin.Unspent {
	if len(slice) == 0 {
		return slice
	}
	for i, v := range slice {
